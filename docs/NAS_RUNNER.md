...
## 지금 상태 — 준비만 해 두고 **아직 옮기지 않았다**

이전은 예식(2026-10-18) 이후로 미뤘다. 이 문서와 `ops/nas/` 는 그때 쓰려고
남겨 둔 것이고, **워크플로는 전부 GitHub 러너에 그대로 있다.**

| | 지금 | 옮긴 뒤 |
|---|---|---|
| `notify-drain` 정기 실행 | GitHub Actions **하루 2회** (월 ~60분) | 나스 DSM 작업 스케줄러 1시간 |
| CI `verify` | GitHub 러너 | 나스 self-hosted |
| CI `db-verify` | GitHub 러너 | 나스 self-hosted (2단계) |
| CI `e2e-db` | GitHub 러너 | 나스 self-hosted (2단계) |

당장 한 것은 **크론을 3시간 → 하루 2회로 낮춘 것**뿐이다 (월 ~240분 → ~60분).
이렇게 낮춰도 되는 이유는 이 워크플로가 실제 드레인을 담당하지 않기 때문이다 —
주 경로는 신청 API 의 `after()`, 하객 브라우저, **Supabase `pg_cron` 5분**이고,
이건 "그 셋이 다 죽었을 때 시끄럽게 알리는" 감시자다.

> **한도가 이미 소진된 동안에는 이것도 안 돈다.** 다음 결제 주기에 자동으로
> 되살아난다. 그 사이 알림 자체는 `pg_cron` 이 계속 처리하므로 주문 알림은
> 정상이다 — 잃는 건 감시뿐이다.

한도가 막혀 있는 동안 CI 세 잡은 계속 빨간불인데, **끄지 않고 남겨 둔다** —
검증을 지워서 초록을 만드는 건 고장을 감추는 것이다. 그동안은 로컬에서 돌린다:
...
`.github/workflows/ci.yml` 의 `verify` 를 이렇게 바꾼다.

```yaml
runs-on: [self-hosted, linux, x64]   # ← ubuntu-latest 에서
timeout-minutes: 30                  # ← 15 에서 (J4125 는 느리다)
...
- run: npx playwright install chromium   # ← --with-deps 를 뺀다
```

⚠️ **러너를 실제로 등록한 뒤에 바꿀 것.** self-hosted 잡은 받아 줄 러너가 없으면
실패가 아니라 `queued` 로 매달려 있다가 24시간 뒤 취소된다. 그러면 Actions 한도가
리셋돼도 그 잡은 영영 돌지 않는다. (2026-09-08 에 순서를 거꾸로 했다가 되돌렸다)
