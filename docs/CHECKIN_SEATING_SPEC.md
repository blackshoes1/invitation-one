...
- Upstash/Vercel WAF rate limit (실제 공격 관찰 시 추가하는 방어 계층)
...
예식장 공용 QR 은 행사 키를 포함한 URL 로 인쇄한다 (개정 2차).

```text
/checkin?t=<개인 UUID>          → 개인 체크인
/checkin?event=<CHECKIN_EVENT_KEY> → 공용 검색 (행사 키 게이트)
파라미터 없음                    → QR 스캔/안내데스크 안내
```

### RSVP 검색

- `?event=` 행사 키가 유효할 때만 검색·체크인 API 가 응답한다
  (키 미설정 시 fail-closed — 익명 인터넷에는 항상 닫힘)
- 이름 + 전화번호 뒤 4자리
- 동명이인이면 전체 전화번호 또는 신랑/신부측으로 추가 확인
- 검색 성공 후 개인 QR 체크인과 같은 흐름으로 진행

### 현장 등록 (안내데스크 전용 — 개정 2차)

- RSVP 없이 방문한 하객은 **안내데스크(관리자 현장운영 탭)** 에서 등록한다.
  공개 walk-in API 는 두지 않는다 — rsvp_id 없는 행을 익명으로 만들 수 있는
  가장 큰 남용 표면이므로 관리자 인증 뒤로 이동.
- 이름 / 신랑측·신부측 / 실제 인원 / 관리자 메모
- `rsvp_id=null`, `source='walk_in'`, `checked_in_by='admin'` 으로 저장
- 공용 QR 페이지는 미예약 하객에게 안내데스크 문구를 보여준다.
...
- **서버 전용 제출 토큰**: 청첩장 페이지(서버 컴포넌트, `?key=` 게이트 통과 후)가
  `sha256('rsvp-submit:' + ADMIN_PASSWORD + ':' + INVITATION_KEY)` 파생 토큰을
  폼 prop 으로 내려주고 API 가 검증한다. JS 번들에는 포함되지 않는다.
  env 미설정 시 fail-closed(503).
- **허니팟 필드** `website`: 값이 있으면 저장 없이 성공 흉내(봇 무력화).
- **총량 상한**: RSVP 행 수 400 초과 시 접수 중단 (스팸 폭주 회로 차단기).
...
## 남용 방어 — 단계적 구조 (개정 2차: Upstash 제거)

IP 기반 rate limit 은 이 서비스에 부적합해 **채택하지 않는다**:

- 예식장 공유 WiFi 에서는 하객 수십 명이 단일 IP 로 몰려, IP 제한이
  정상 하객 체크인을 차단하는 최악의 실패 모드가 된다.
- 개인 QR 은 토큰 자체가 128비트 비밀이라 rate limit 이 더하는 방어가
  거의 없고, limiter 장애가 당일 체크인을 막는 위험만 추가한다.

대신 공개 표면별로 키·토큰·DB 제약으로 방어한다:

| 표면 | 방어 |
|---|---|
| 개인 QR 조회·체크인 | UUID 토큰(비밀) + 운영 시간 + 활성 1건 unique |
| 공용 검색·체크인 | `CHECKIN_EVENT_KEY`(?event=, 서버 전용 env, 미설정 시 fail-closed) + 이름·뒤4자리 + 운영 시간 + unique |
| RSVP 제출 | 서버 전용 제출 토큰(키 게이트를 통과한 청첩장 페이지만 폼에 내려줌 — ADMIN_PASSWORD 파생, 번들 미포함) + 허니팟 필드 + 입력 길이 제한 + 전체 RSVP 최대 건수(회로 차단기) |
| 현장 등록(walk-in) | **공개 API 없음** — 관리자(안내데스크) 전용 `/api/admin/checkins` |

- 관리자 API는 기존 관리자 인증 적용.
- Upstash/Vercel WAF 등 rate limit 은 **실제 공격이 관찰될 때 추가하는
  후속 방어 계층**으로 남긴다 (후속 기능).
- 추가 환경변수: `CHECKIN_EVENT_KEY` (서버 전용, `.env.local.example` 참고)
...
- 서버 전용 제출 토큰 없는 RSVP 제출이 403 으로 거부된다.
- 행사 키 없는 공용 검색·체크인이 거부된다 (키 미설정 시 fail-closed).
- 공개 경로로는 walk-in 체크인을 생성할 수 없다 (관리자 API 전용).
...
8. 남용 방어 구성 — 제출 토큰·행사 키·허니팟·총량 상한 (+ `.env.local.example`에 CHECKIN_EVENT_KEY 반영)
