...
> **구현됨 (2026-09-06).** 아래 A안 그대로 반영했다 — `src/lib/telegram.ts`,
> `src/lib/notifyChannel.ts`, `notifyOutbox.ts` 어댑터 교체, 안전망 워크플로 판정
> (`ready` / `channelStatus`), 어드민 배너. 남은 것은 봇 토큰·chat_id 를 Vercel
> 환경변수에 넣는 일뿐이다. 이 문서는 배경과 판단 근거로 남긴다.
