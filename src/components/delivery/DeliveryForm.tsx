...
export default function DeliveryForm() {
  ...
  const next = () => {
    ...
    if (step === 2 && !location.trim()) return setError("배달 받으실 곳을 입력해주세요 📍");
    if (step === 3 && !date) return setError("배달 희망일을 골라주세요 📅");
  ...
  const submit = async () => {
    ...
    if (isSupabaseConfigured && supabase) {
      ...
      if (error) {
        setSending(false);
        if (error.code === "23505") {
          setError("앗, 이 날짜는 품절이에요! 다른 날짜를 골라주세요 🙏");
          setDate(null);
          setDir(-1);
          setStep(3);
          loadBooked();
  ...
  if (done && date && slot) {
    return <CompletePage name={name} date={date} slot={slot} location={location} />;
  }
  ...
  const content = () => {
    switch (step) {
      case 0:
        return (
          <Q title="주문자 성함을 알려주세요 📝" sub="배달 주문을 시작할게요!">
            <input
              autoFocus
              value={name}
      ...
      case 1:
        return (
          <Q
            title={`${name || "고객"}님, 어디로 알림 보낼까요? 📱`}
            sub="배차되면 이 번호로 연락드려요"
          >
            <input
              autoFocus
              type="tel"
      ...
      case 2:
        return (
          <Q title="어디로 배달해드릴까요? 📍" sub="전국 어디든 배달 가능! 🛵">
            <input
              autoFocus
              value={location}
      ...
      case 3:
        return (
          <Q title="배달 희망일을 골라주세요 📅" sub="품절(취소선)된 날짜는 선택할 수 없어요">
            <DeliveryCalendar
              selected={date}
              booked={booked}
              onSelect={setDate}
      ...
      case 4:
        return (
          <Q title={`${date ? formatYmdKo(date) : ""}, 몇 시쯤이 좋을까요? ⏰`}>
      ...
      case 5:
        return (
          <Q title="배달 요청사항 있으신가요? 💬" sub="“문 앞에서 벨 눌러주세요” 같은 거요 (선택)">
            <textarea
              autoFocus
              value={message}
